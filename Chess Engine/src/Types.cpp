...
{
	/* Methods */
	uint64_t SetBit(const uint64_t bitboard, const uint8_t square) { return bitboard | (1ULL << square); }
	// Set the bit at the Square
	uint64_t ClearBit(const uint64_t bitboard, const uint8_t square) { return bitboard & ~(1ULL << square); }
	// Clear the bit at the Square
	uint64_t GetBit(const uint64_t bitboard, const uint8_t square) { return bitboard & (1ULL << square); }
	// Get the bit at the Square
	uint64_t PopBit(const uint64_t bitboard, const uint8_t square)
	{
		return GetBit(bitboard, square) | ClearBit(bitboard, square);
	} // Pop the bit at the Square
	uint64_t CountBits(const uint64_t bitboard) { return __popcnt64(bitboard); }
	// Count the number of bits in the bitboard
	...
	void PrintMoveSquare(const uint32_t move)
	{
		const uint8_t initialPosition = (move >> 19) & 0b111111;
		const uint8_t finalPosition = (move >> 13) & 0b111111;
		...
		std::cout << squares[initialPosition] << squares[finalPosition];
	}
